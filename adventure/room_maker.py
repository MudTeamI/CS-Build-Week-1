        ...
        for y in range(self.y):
            for x in range(self.x):
                if (x+1 < self.x) and (self.tracker[y][x+1] == (self.tracker[y][x] + 1)):
                    self.maze_room[y][x].connectRooms(
                        self.maze_room[y][x+1], 'e')
                    self.maze_room[y][x +
                                      1].connectRooms(self.maze_room[y][x], 'w')
                elif (x-1 >= 0) and (self.tracker[y][x-1] == (self.tracker[y][x] + 1)):
                    self.maze_room[y][x].connectRooms(
                        self.maze_room[y][x-1], 'w')
                    self.maze_room[y][x -
                                      1].connectRooms(self.maze_room[y][x], 'e')
                elif (y+1 < self.y) and (self.tracker[y+1][x] == (self.tracker[y][x] + 1)):
                    self.maze_room[y][x].connectRooms(
                        self.maze_room[y+1][x], 's')
                    self.maze_room[y +
                                   1][x].connectRooms(self.maze_room[y][x], 'n')
                elif (y-1 >= 0) and (self.tracker[y-1][x] == (self.tracker[y][x] + 1)):
                    self.maze_room[y][x].connectRooms(
                        self.maze_room[y-1][x], 'n')
                    self.maze_room[y -
                                   1][x].connectRooms(self.maze_room[y][x], 's')
